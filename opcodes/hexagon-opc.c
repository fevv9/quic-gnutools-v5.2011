...
const hexagon_operand hexagon_operands [] =
{
  ...
  { "#g16:1ie", 6,    16, 'i', 0,
    ...
    "#%g", "#0x%x", NULL },
  { "#g16:1", 0,    16, 'i', 1,
    BFD_RELOC_HEX_GPREL16_1, BFD_RELOC_HEX_IE_32_6_X, BFD_RELOC_HEX_IE_16_X,
    HEXAGON_OPERAND_IS_IMMEDIATE,
    "#%g", "#0x%x", NULL },
  { "#g16:1ie", 6,    16, 'i', 1,
    BFD_RELOC_NONE, BFD_RELOC_HEX_32_6_X, BFD_RELOC_HEX_16_X,
  ...
  { "#s16gotrel", 4, 16, 'i', 0,
    ...
    "#%d", NULL, NULL },
  { "#s16gdgot", 4,  16, 'i', 0,
    BFD_RELOC_HEX_GD_GOT_16, BFD_RELOC_HEX_GD_GOT_32_6_X, BFD_RELOC_HEX_GD_GOT_16_X,
    HEXAGON_OPERAND_IS_IMMEDIATE | HEXAGON_OPERAND_IS_SIGNED,
    "#%d", NULL, NULL },
  { "#s16ie", 4,     16, 'i', 0,
    BFD_RELOC_NONE, BFD_RELOC_HEX_IE_32_6_X, BFD_RELOC_HEX_IE_16_X,
    ...
    "#%d", NULL, NULL },
  { "#s16iegot", 4,  16, 'i', 0,
    BFD_RELOC_HEX_IE_GOT_16, BFD_RELOC_HEX_IE_GOT_32_6_X, BFD_RELOC_HEX_IE_GOT_16_X,
    HEXAGON_OPERAND_IS_IMMEDIATE | HEXAGON_OPERAND_IS_SIGNED,
    "#%d", NULL, NULL },
  { "#s16dtprel", 4, 16, 'i', 0,
    BFD_RELOC_HEX_DTPREL_16, BFD_RELOC_HEX_DTPREL_32_6_X, BFD_RELOC_HEX_DTPREL_16_X,
    HEXAGON_OPERAND_IS_IMMEDIATE | HEXAGON_OPERAND_IS_SIGNED,
    "#%d", NULL, NULL },
  { "#s16tprel", 4,  16, 'i', 0,
    BFD_RELOC_HEX_TPREL_16, BFD_RELOC_HEX_TPREL_32_6_X, BFD_RELOC_HEX_TPREL_16_X,
    HEXAGON_OPERAND_IS_IMMEDIATE | HEXAGON_OPERAND_IS_SIGNED,
    "#%d", NULL, NULL },
  { "#s12", 0,      12, 'i', 0,
  ...
  { "#u26:6", 0,    26, 'i', 6,
    ...
    HEXAGON_OPERAND_IS_IMMEDIATE | HEXAGON_OPERAND_IS_SIGNED | HEXAGON_OPERAND_IS_KXER,
    "#%u", NULL, NULL },
  { "#u26:6", 0,    26, 'i', 6,
 ...
 char **errmsg)
{
  ...
  value.s = is_s? (int32_t) avalue: avalue;
  ...
  if (operand->flags & HEXAGON_OPERAND_IS_LO16)
    ...
    value.s = HEXAGON_SUBREGS_TO
		(value.s, operand->flags & HEXAGON_OPERAND_IS_PAIR);
  ...
  smax = ~(~0 << (bits - 1));
  smin =  (~0 << (bits - 1))
	  + ((operand->flags & HEXAGON_OPERAND_IS_NEGATIVE)? 1: 0);
  umax = ~(~0U << bits);
  umin = 0U;
  ...
  for (i = 0; i < hexagon_operand_count; i++)
    {
      if (flags & HEXAGON_OPERAND_IS_KXED)
          ...
          if (hexagon_operands [i].flags & HEXAGON_OPERAND_IS_IMMEDIATE)
	    {
	      ...
	      f = strdup (hexagon_operands [i].fmt);
	      if (hexagon_operands [i].len)
		f [hexagon_operands [i].len] = 0;

	      if (opcode->attributes & EXTENDABLE_LOWER_CASE_IMMEDIATE)
		{
		  if (hexagon_operands [i].enc_letter
		      != TOLOWER (hexagon_operands [i].enc_letter)
		      || !strstr (opcode->syntax, f))
		    continue;
		}
	      else if (opcode->attributes & EXTENDABLE_UPPER_CASE_IMMEDIATE)
		{
		  if (hexagon_operands [i].enc_letter
