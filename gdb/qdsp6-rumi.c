...
static void qdsp6rumi_close (int quitting);
static void qdsp6_mourn_inferior (void);
struct target_ops *qdsp6rumi_target = (struct target_ops *)NULL;
static int portid = 0;
...
/*
 * function: qdsp6rumi_exec_simulation
 * description:
 * 	Call fork/execvp to start the simulator
 */
static int
qdsp6rumi_exec_simulation(char *sim_name, char *exec_name,
...
/*
 * function: qdsp6rumi_create_inferior
 * description:
 *        - Primary function of this target.
 *        - Starts the simulator and forms a connection.
 */
static void qdsp6rumi_create_inferior (char *exec_file, char *args, char **env)
{
    ...
    qdsp6rumi_exec_simulation("qdsp6-rumi", exec_bfd->filename, portid, NULL, NULL);
...
/*
 * function: qdsp6rumi_open
 * description:
 *      - Called when the user selects the qdsp6-rumi target architecture.
 */
static void
qdsp6rumi_open (char *args, int from_tty)
{
    extern char *current_q6_target;
    if (qrumi_debug)
    {
    ...
    target_preopen (1);
    current_q6_target = "qdsp6-rumi";
}


void
_initialize_qdsp6_rumi (void)
{
  struct target_ops *t;

  t = XZALLOC (struct target_ops);

  t->to_shortname = "qdsp6-rumi";
  t->to_longname = "Qualcomm Hexagon RUMI interface";
  t->to_doc = "Qualcomm Hexagon Rumi interface, used for debugging software \n\
running on the Hexagon (qdsp6-rumi) target";

  t->to_open = qdsp6rumi_open;
  t->to_close = qdsp6rumi_close;
  t->to_can_run = qdsp6rumi_can_run;
  t->to_create_inferior = qdsp6rumi_create_inferior;
