...
void  hexagon_blurb PARAMS ((FILE *));
void
hexagon_blurb (file)
     FILE *file;
{
  ...
  fprintf (file, " %c/call\t\tfunction per call, if this function is profiled,\n", hist_dimension_abbrev);
  fputs ("\t\telse blank.\n", file);
  fputs ("\n", file);
  fprintf (file, " total\t\tthe average number of %s spent in this\n", hist_dimension);
  fprintf (file, " %c/call\t\tfunction and its descendents per call, if this \n", hist_dimension_abbrev);
