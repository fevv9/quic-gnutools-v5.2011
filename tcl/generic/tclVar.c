    ...
    Tcl_Obj *CONST objv[];		/* Argument objects. */
{
	...
	return TCL_OK;
     ...
     ptr2varName  = Tcl_GetString(objv[1]);
   
    /* [ams] Check to see if t_set tries to set a gdb keyword*/
    if(ptr2varName != NULL) {
     if(LookUPGDBDollarVar(ptr2varName) != NULL)
      {
        printf("\"%s\" is a gdb keyword.Cannot re-use it as Tcl variable name !\n",
               ptr2varName);
        return TCL_ERROR;
      }
    }
